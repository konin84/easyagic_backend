...
- **`apps/products`** — Catalogue listed in the app. One `Product` model holds both `kind`s: `input` (seed/fertilizer/crop_protection/tool/irrigation/protective_equipment) and `produce` (grain/tuber/legume/vegetable/fruit/cash_crop), with `CATEGORIES_BY_KIND` pinning which categories belong to which kind — the seeder rejects a mismatch. Artwork resolves through `display_image` in strict order — an admin-uploaded photo, then `image_url` (all 38 products carry a freely-licensed Wikimedia Commons photo, each visually reviewed and HTTP-verified, with the licence attribution in `image_credit` because most are CC BY-SA), then generated placeholder artwork (still the safety net for any entry added without a photo; the ten products for which no suitable photo existed were dropped from the catalogue in migration `0003`, since the seeder never deletes rows on its own) (`images.py`: an 800x600 category-coloured card carrying the product name, so nothing renders as a broken tile before real photography exists). Generated art is written under `products/generated/`, which is what lets the seeder distinguish it from a genuine admin upload — an uploaded photo is never overwritten, not even by `--regenerate-images`. Font lookup degrades to PIL's built-in face if the container ships no TrueType fonts. Each row carries a two-sentence practical description, a sales `unit`, and an image resolved from an uploaded `ImageField` first, then `image_url`, then null; the serializer flattens that into one `image` field so the app has no branching. 38 products ship in `apps/products/data/products.json`, loaded by `seed_products` (idempotent, create-not-overwrite, `--file`/`--overwrite`, wired into `render.yaml` alongside `seed_payment_config`). `translation.py` serves the catalogue in the reader's language: `name`/`description`/`unit` are translated once per language and cached in a `translations` JSON column, the 14 fixed kind/category labels sit in Django's cache, and `slug`/`kind`/`category` deliberately stay English since they are filter keys. A `translations_source` fingerprint of the English text invalidates cached translations when someone rewords a description, and a failed API call is never cached as English (`translate_batch` returns its input on failure, so identity is checked). `translate_products` pre-warms languages so no farmer pays the first-call latency. Read-only endpoints, authenticated: `GET /api/products/` (filters `kind`/`category`/`search`, `limit`/`offset` paging, `{count, limit, offset, results}`; `category` is resolved by `resolve_category` from the code, the English label, or the translated label, so a client holding only the display name from `/categories/` can send it back verbatim — an unrecognised value returns 400 listing the valid options rather than silently returning nothing), `GET /api/products/categories/` (chips grouped by kind with live counts so no filter is ever empty), `GET /api/products/by-category/` (the whole catalogue in one request; `group_by` composes — `kind` (default) nests in a curated order, `kind,name` nests by kind *and* sorts categories alphabetically inside each, `name` returns one flat list — and is shared with `/categories/` via `parse_group_by`, so the chips row and the sectioned list can never disagree on order; sorting always uses the *translated* name with accent-insensitive collation so Équipement files under E; `limit_per_category` trims each group for a home screen while `count` stays the true total, and the whole catalogue is translated in a single pass rather than once per group), `GET /api/products/<identifier>/` (the URL uses the `str` converter, not `slug`: the slug converter rejects spaces and punctuation, so only single-word lowercase names resolved and everything else 404'd before reaching the view. The view now resolves the slug, `slugify(identifier)`, the numeric id, or a case-insensitive name; a name containing a slash remains unaddressable by name since it cannot appear in a path segment). Covered by `apps/products/tests.py` (122 tests), including one asserting every category has products.
...
- JWT via `rest_framework_simplejwt`: **24-hour access** (`JWT_ACCESS_HOURS`), 7-day refresh (`JWT_REFRESH_DAYS`), rotation + blacklist-after-rotation. Access tokens are not revocable — only refresh tokens can be blacklisted — so deactivating or deleting an account leaves its existing access token working until it expires, now up to 24 hours rather than 30 minutes.
