...
- **`apps/products`** — Catalogue listed in the app. One `Product` model holds both `kind`s: `input` (seed/fertilizer/crop_protection/tool/irrigation/protective_equipment) and `produce` (grain/tuber/legume/vegetable/fruit/cash_crop), with `CATEGORIES_BY_KIND` pinning which categories belong to which kind — the seeder rejects a mismatch. Each row carries a two-sentence practical description, a sales `unit`, and an image resolved from an uploaded `ImageField` first, then `image_url`, then null; the serializer flattens that into one `image` field so the app has no branching. 48 products ship in `apps/products/data/products.json`, loaded by `seed_products` (idempotent, create-not-overwrite, `--file`/`--overwrite`, wired into `render.yaml` alongside `seed_payment_config`). Read-only endpoints, authenticated: `GET /api/products/` (filters `kind`/`category`/`search`, `limit`/`offset` paging, `{count, limit, offset, results}`), `GET /api/products/categories/` (chips grouped by kind with live counts so no filter is ever empty), `GET /api/products/<slug>/`. Covered by `apps/products/tests.py` (21 tests), including one asserting every category has products.
