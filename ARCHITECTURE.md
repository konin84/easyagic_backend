...
- **`apps/products`** — Catalogue listed in the app. One `Product` model holds both `kind`s: `input` (seed/fertilizer/crop_protection/tool/irrigation/protective_equipment) and `produce` (grain/tuber/legume/vegetable/fruit/cash_crop), with `CATEGORIES_BY_KIND` pinning which categories belong to which kind — the seeder rejects a mismatch. Artwork resolves through `display_image` in strict order — an admin-uploaded photo, then `image_url` (38 of 48 products carry a freely-licensed Wikimedia Commons photo, each visually reviewed and HTTP-verified, with the licence attribution in `image_credit` because most are CC BY-SA), then generated placeholder artwork (`images.py`: an 800x600 category-coloured card carrying the product name, so nothing renders as a broken tile before real photography exists). Generated art is written under `products/generated/`, which is what lets the seeder distinguish it from a genuine admin upload — an uploaded photo is never overwritten, not even by `--regenerate-images`. Font lookup degrades to PIL's built-in face if the container ships no TrueType fonts. Each row carries a two-sentence practical description, a sales `unit`, and an image resolved from an uploaded `ImageField` first, then `image_url`, then null; the serializer flattens that into one `image` field so the app has no branching. 48 products ship in `apps/products/data/products.json`, loaded by `seed_products` (idempotent, create-not-overwrite, `--file`/`--overwrite`, wired into `render.yaml` alongside `seed_payment_config`). Read-only endpoints, authenticated: `GET /api/products/` (filters `kind`/`category`/`search`, `limit`/`offset` paging, `{count, limit, offset, results}`), `GET /api/products/categories/` (chips grouped by kind with live counts so no filter is ever empty), `GET /api/products/<slug>/`. Covered by `apps/products/tests.py` (41 tests), including one asserting every category has products.
